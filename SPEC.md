...
- `hooks/curate.py`, running detached, then:
  ...
  2. Checks if the transcript is below the small-signal threshold (< 3 user turns OR < 1500 chars of user content). If so, records `skip_reason_a = skip_reason_b = "threshold"` in the log entry and exits 0. JSONL parsing runs here, in Python — not in the shell script.
  ...
  6. Runs `scrub.py` again on each model output (defense-in-depth — models fed scrubbed input shouldn't re-emit secrets, but re-scrubbing is cheap insurance).
  7. **Sanitizes the model-provided `title`** before using it in filenames, frontmatter, or wikilinks: strip `|`, `]]`, `[[`, `#`, backticks, control chars, and leading/trailing whitespace; collapse internal whitespace; truncate to 120 chars. Rationale: the title is model-generated (untrusted input at this boundary) and flows into `[[stem|title]]` wikilinks in devlog/recap notes (§2.3); an unsanitized title would corrupt those notes.
  8. Derives `<slug>` deterministically from the sanitized title: NFKD-normalize and strip non-ASCII; lowercase; replace every run of non-`[a-z0-9]+` chars with a single `-`; strip leading/trailing `-`; truncate to 60 chars (at a `-` boundary if possible). If the result is empty (title was all punctuation), fall back to `untitled`. Path A and Path B each derive their slug from their own model-provided title — slugs are not shared across paths.
  9. Writes non-null Path A output to `Inbox/auto/YYYY-MM-DD-<slug>-<sid8>.md`, where `<sid8>` is the first 8 chars of the session id (disambiguates same-day same-title collisions).
  10. Writes Path B output to `Inbox/raw/YYYY-MM-DD-<slug>-<sid8>.md`.
  11. Appends one line to `eval/state/session-index.tsv`: `<session_id>\t<path_a_or_null>\t<path_b_or_null>\t<YYYY-MM-DD>`. This is the fast lookup used by dedup and miss-rate checks — no vault walk required after the index exists. The file starts with a single `# schema_version: 1` comment line (skipped by the dedup scan) on first creation.
  12. Appends one record to `eval/state/log.md` (schema in §2.2).
...
| `skip_reason_a` | string \| `null` | `null` when `path_a` is a path. Otherwise one of: `"model_returned_null"`, `"threshold"`, `"token_limit"`, `"timeout"`, `"error:<type>"` |
| `skip_reason_b` | string \| `null` | `null` when `path_b` is a path. Otherwise one of: `"threshold"`, `"token_limit"`, `"timeout"`, `"malformed_json"`, `"error:<type>"` |
...
**No-capture session definition** (drives the 25% alarm in §2.3): `path_a IS null AND path_b IS null AND skip_reason_a != "threshold"`. Threshold skips are excluded from *both* the numerator and the denominator of the alarm ratio — they are expected behaviour, not failure.
...
- Body follows a fixed template:
  ...
  *(populated by skill on promotion; section omitted until first promotion)*
  ```
- No filtering — if the session did anything meaningful it gets summarized. Cheap and consistent. Path B always returns structured JSON by prompt design. If the model returns malformed JSON despite this, `curate.py` logs the raw output and records `skip_reason_b: "malformed_json"` — same failure-isolation treatment as a Path A error. This alone does not count toward the no-capture alarm unless `skip_reason_a` is also non-threshold (per §2.2 no-capture definition).
...
- Python (`curate.py`): single file, stdlib + `anthropic` SDK only. No classes unless state demands it. Read args → scrub transcript → threshold check → token-count guard (`CAPTURE_MAX_EST_TOKENS`) → check session-index (dedup) → two parallel API calls via `concurrent.futures.ThreadPoolExecutor(max_workers=2)` → scrub outputs → sanitize title → write up to two files → append index + log → exit 0. Errors go to stderr and `~/.claude/hooks.log`, never to Claude Code's UI.
